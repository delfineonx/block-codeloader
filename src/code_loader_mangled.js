...
}
{
  ...
  V.h=()=>{
    ...
    G=CF_.EVENT_REGISTRY;
    A=CF_.ACTIVE_EVENTS;
    let a=CF_.event_manager,
    b=!!a.is_interruption_framework_enabled,
    c=a.default_retry_delay_ticks|0;
    c=c&~(c>>31);
    let d=a.default_retry_limit_ticks|0;
    d=(d&~(d>>31))+(-d>>31)+1;
    let e=V.c,
    f=V.d,
    g=V.e,
    ...
    while(i<h){
      ...
      if(b&m){
        ...
        let p=IF_;
        e[k]=N;
        globalThis[k]=function handler(a0,a1,a2,a3,a4,a5,a6,a7,a8){
          p.state=1;
          p.handler=handler;
          p.args=[a0,a1,a2,a3,a4,a5,a6,a7,a8];
          p.delay=n;
          p.limit=o;
          p.phase=400000;
          try{
            return e[k](a0,a1,a2,a3,a4,a5,a6,a7,a8)
          }finally{
            p.state=0
          }
        }
        ...
        e[k]=N;
        globalThis[k]=function(a0,a1,a2,a3,a4,a5,a6,a7,a8){
          return e[k](a0,a1,a2,a3,a4,a5,a6,a7,a8)
        }
      }
      i++
  ...
  V.l=()=>{
    ...
    while(S<b){
      ...
      if(a[d]){
        ...
        api.setCallbackValueFallback(d,e)
      }else{
        V.f[V.f.length]=d
      }
      S++
...
}
{
  ...
  B.f=()=>{
    ...
    B.c=CF_.BLOCKS instanceof Array?CF_.BLOCKS.slice():[];
    let a=CF_.block_manager;
    C=!!a.default_locked_status;
    A=!!a.default_execution_status;
    I=a.max_registrations_per_tick|0;
    I=(I&~(I>>31))+(-I>>31)+1;
    Q=a.max_chunk_requests_per_tick|0;
    Q=(Q&~(Q>>31))+(-Q>>31)+1;
    V=a.max_executions_per_tick|0;
    V=(V&~(V>>31))+(-V>>31)+1;
  ...
  let U=()=>{
    ...
    while(X<c&d>0){
      ...
      try{
        let f=api.getBlockData(x,y,z)?.persisted?.shared?.text;
        (0,eval)(f)
      }catch(g){
        E++;
        b[E*+(b.length-1<O)]=[x,y,z,g.name,g.message]
      }
      d--;
      X++
    }
  ...
  B.h=a=>{
    ...
    y=a[1],
    z=a[2],
    b=(x|0)-(x<(x|0))+"|"+((y|0)-(y<(y|0)))+"|"+((z|0)-(z<(z|0)));
    if(K[b]!==!1){
      return!Y.c
    }
    return !1
...
}  
{
  ...
  Y.f=a=>{
    let b=B.d.length-1,
    c="Code Loader: BootManager: Code was loaded in "+T*50+" ms";
    if(b===0){
      c+=a?" with 0 errors.":".";
      api.broadcastMessage([{
        str:c,
  ...
  Y.g=()=>{
    ...
    c="Code Loader: BlockManager: Executed "+d+" block"+(d===1?"":"s")+":"+c;
    api.broadcastMessage([{
      str:c,
      style:(B.d.length-1?Y.d.warning:Y.d.success)??{}
    }])
  };
  Y.h=()=>{
    let a=B.d,
    b=a.length-1;
    if(b>0){
      let c="Code Loader: BlockManager: Code execution error"+(b===1?"":"s")+":",
      d=1;
      while(d<=b){
        let e=a[d];
  ...
  Y.e=(a,b,c)=>{
    if(V.e.length){
      api.broadcastMessage([{
        str:"Code Loader: EventManager: Unregistered active events: \""+V.e.join("\", \"")+"\".",
        style:Y.d.warning??{}
      }])
    }
    if(V.f.length){
      api.broadcastMessage([{
        str:"Code Loader: EventManager: Invalid active events: \""+V.f.join("\", \"")+"\".",
        style:Y.d.warning??{}
      }])
    }
    if(a){
...
}
{
  ...
  C.configuration=CF_;
  C.isBlockLocked=a=>{
    return a instanceof Array&&a.length===3&&B.h(a)
  };
  C.reboot=()=>{
    if(!Y.c){
      V.c.tick=Y.i;
...
try{
  ...
  Object.freeze(CF_.EVENT_REGISTRY);
  EM_.b=!0;
  OM_.a=0
}catch(error){
  EM_.a=[error.name,error.message]
}
globalThis.IF=IF_;
globalThis.CL=CL_;
void 0;
